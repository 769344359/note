...
- 0x907a  
tcp 头的前两个字节 一共16位是源端口,本例子中的0x907a 就是源端口  

- 0x0050   
第三四个字节的是目的端口,本例子中的0x0050就是目的端口,其实就是80端口  

- 0xc7f751cc

第5到第8字节组成了序列号,本例子中的 0xc7f751cc 就是序列号, tcp 序列号标识该tcp 报文的报文体的第一个字节的编号

- 0x0000 0000 

第9-12 字节代表的是 ack 号,本例子是 0


- 0xa002 
第十三到第十四个字节的前4位 代表数据偏移，4bits，单位为4字节，它指出报文数据距TCP报头的起始处有多远(TCP报文头长度)  
本例子中是 a 即 10*4 = 40 字节  
中间6位置0  
后面6位是标志位 ,本例子是2 即000010 即是syn 标志位的包  

-  aaaa   
第十五十六字节是代表自己窗口大小 本例子是 0xaaaa    
